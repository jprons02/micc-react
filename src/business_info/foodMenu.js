//Our little shack (The Village)
export const ourLittleShackMenu = [
  {
    name: "Breakfast",
    subMenu: [
      {
        name: "Main",
        items: [
          { name: "Pancakes with bacon or sausage", price: "$6.00" },
          { name: "Bacon & Eggs", price: "$6.00" },
          { name: "Sausage & Eggs", price: "$6.00" },
          { name: "Spam & Eggs", price: "$6.00" },
          { name: "Smoked Sausage with fries", price: "$4.00" },
          { name: "Custom Omelet", price: "$6.00" },
          { name: "French Toast with bacon", price: "$6.00" },
          { name: "Grits", price: "$3.00" },
          { name: "Home Fries", price: "$3.00" },
          { name: "Hash Browns", price: "$3.00" },
          { name: "Bagel", price: "$2.00" },
          { name: "Corn Beef Hash", price: "$3.00" },
        ],
      },
    ],
  },
  {
    name: "Lunch",
    subMenu: [
      {
        name: "Main",
        items: [
          { name: "Cheeseburger", price: "$7.00" },
          { name: "Bacon Cheeseburger", price: "$8.00" },
          { name: "Buffalo Burger", price: "$10.00" },
          { name: "Gator Bites", price: "$15.00" },
          { name: "Frog Legs", price: "$20.00" },
          { name: "Shrimp", price: "$10.00" },
          { name: "Grilled Chicken Sandwich", price: "$7.00" },
          { name: "Chicken or Steak Quesadilla", price: "$7.00" },
          { name: "Chicken Tenders (5 pc)", price: "$7.00" },
          { name: "Chicken Wings (7 pc)", price: "$8.00" },
          { name: "Grilled Cheese Sandwich", price: "$5.00" },
          { name: "Chili Dog", price: "$5.00" },
          { name: "Chicken nuggets (5pc)", price: "$5.00" },
          { name: "Club Sandwich (Sandwich only, $5)", price: "$9.00" },
          { name: "BLT Sandwich (Sandwich only, $4)", price: "$8.00" },
          { name: "Steak Sandwich", price: "$7.00" },
          { name: "Chicken Wrap", price: "$7.00" },
          { name: "Steak Wrap", price: "$8.00" },
        ],
      },
      {
        name: "Sides",
        items: [
          { name: "French Fries", price: "$3.00" },
          { name: "Mozarella Sticks", price: "$3.00" },
          { name: "Sweet Potato Fries", price: "$4.00" },
          { name: "Onion Rings", price: "$3.00" },
        ],
      },
      {
        name: "Salads",
        items: [
          { name: "Salad with chicken", price: "$7.00" },
          { name: "Salad with shrimp", price: "$8.00" },
          { name: "Salad with steak", price: "$9.00" },
        ],
      },
    ],
  },
  {
    name: "Kid's Menu",
    items: [
      { name: "Mac & Cheese", price: "$3.00" },
      { name: "Hotdog", price: "$4.00" },
      { name: "Pizza", price: "$5.00" },
    ],
  },
  {
    name: "Drinks",
    items: [
      { name: "Red Bull", price: "$3.00" },
      { name: "Milkshakes (Vanilla/Chocolate)", price: "$4.00" },
      { name: "Coffee", price: "$2.00" },
      { name: "Cuban Coffee", price: "$1.50" },
      { name: "Water", price: "$1.00" },
      { name: "Soda", price: "$1.00" },
    ],
  },
];

// Sawgrass Cafe
export const sawgrassCafeMenu = [
  {
    name: "Drinks",
    subMenu: [
      {
        name: "Cold Beverages",
        items: [
          { name: "Soda Can (12 oz.)", price: "$1.18" },
          { name: "Soda Bottle (20 oz.)", price: "$2.06" },
          { name: "Fountain Soda", price: "$1.42" },
          { name: "Monster", price: "$2.28" },
          { name: "Powerade", price: "$2.06" },
          { name: "Dunkin Donuts Beverages", price: "$2.18" },
          { name: "Gold Peak Tea", price: "$2.06" },
          { name: "Water Bottle", price: "$1.50" },
          { name: "Juice", price: "$2.58" },
        ],
      },
      {
        name: "Hot Beverages",
        items: [
          { name: "Hot Tea", price: "$1.00" },
          { name: "American Coffee", price: "$1.84" },
          { name: "Colada", price: "$2.00" },
          { name: "Cortadito", price: "$2.20" },
          { name: "Café con Leche", price: "$3.30" },
          { name: "Hot Chocolate", price: "$3.30" },
          { name: "Cappuccino", price: "$3.80" },
        ],
      },
    ],
  },
  {
    name: "Appetizers",
    items: [
      { name: "Croquetas", price: "$1.25" },
      { name: "Toast", price: "$1.65" },
      { name: "Pan de Bono", price: "$1.90" },
      { name: "Cuban Toast", price: "$2.25" },
      { name: "Pastelitos", price: "$2.20" },
      { name: "Papa Rellena", price: "$2.50" },
      { name: "Tequeño", price: "$2.50" },
      { name: "Empanadas", price: "$2.75" },
    ],
  },
  {
    name: "Combos",
    subMenu: [
      {
        name: "Include 6 oz. French Fries and 16 oz. Fountain Soda ",
        items: [
          { name: "Soup, Ham & Cheese Sandwich", price: "$8.00" },
          { name: "Soup, BLT Sandwich", price: "$8.00" },
          { name: "Soup, Turkey & Provolone Sandwich", price: "$8.00" },
          { name: "Soup, Grilled Cheese Sandwich", price: "$8.00" },
          { name: "Soup, Tuna Salad Sandwich", price: "$8.00" },
          { name: "Soup, Chicken Salad Sandwich", price: "$8.00" },
          { name: "Soup & Salad", price: "$8.00" },
        ],
      },
    ],
  },
  {
    name: "Hot Stuff",
    items: [
      { name: "Hot Dog & Fries", price: "$6.50" },
      { name: "Chicken Tenders", price: "$10.50" },
      { name: "Chicken Wings", price: "$12.50" },
    ],
  },
  {
    name: "Salad",
    items: [
      { name: "Mac Salad Scoop", price: "$3.85" },
      { name: "Chicken Salad Scoop", price: "$5.75" },
      { name: "Tuna Salad Scoop", price: "$6.00" },
      { name: "Garden Salad", price: "$7.00" },
    ],
  },
  {
    name: "Sandwiches",
    items: [
      { name: "Grilled Cheese", price: "$6.50" },
      { name: "Chicken Salad Sandwich", price: "$7.50" },
      { name: "BLT Sandwich", price: "$8.00" },
      { name: "Ham & Cheese Sandwich", price: "$8.00" },
      { name: "Turkey & Provolone Sandwich", price: "$8.00" },
      { name: "Croqueta Preparada", price: "$8.25" },
      { name: "Tuna Salad Sandwich", price: "$8.50" },
      { name: "Hamburger", price: "$8.50" },
      { name: "Chicken Sandwich", price: "$9.00" },
      { name: "Cuban Sandwich", price: "$9.00" },
      { name: "Cheeseburger", price: "$9.50" },
      { name: "Club Sandwich", price: "$9.50" },
      { name: "Steak Palomilla Sandwich", price: "$10.50" },
    ],
  },
  {
    name: "Sides",
    items: [{ name: "French Fries (8 oz.)", price: "$3.00" }],
  },
  {
    name: "Soup",
    items: [
      { name: "Miccosukee Chicken Soup", price: "$5.50" },
      { name: "Soup of the Day", price: "$5.50" },
    ],
  },
  {
    name: "Pizza",
    items: [
      { name: "Cheese Pizza (Slice)", price: "$3.50" },
      { name: "Pepperoni Pizza (Slice)", price: "$3.75" },
      { name: "Whole Pizza", price: "$19.00" },
    ],
  },
];
